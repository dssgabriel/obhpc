...
matrix_t *matrix_init(const size_t rows, const size_t cols) {
    ...
    m->rows = rows;
    m->cols = cols;
    m->data = (f64**)(aligned_alloc(ALIGNMENT, rows * sizeof(f64)));
	if (!m->data) {
    	return NULL;
	}

	for (size_t i = 0; i < rows; i++) {
    	m->data[i] = (f64*)(aligned_alloc(ALIGNMENT, cols * sizeof(f64)));
	}
	if (!*m->data) {
    	return NULL;
	}
...
matrix_t *matrix_rand_init(const size_t rows, const size_t cols) {
	...
	for (size_t i = 0; i < m->rows; i++) {
    	for (size_t j = 0; j < m->cols; j++) {
        	f64 value = ((f64)(rand() / (f64)(RAND_MAX)) * m->rows * m->cols);
        	u8 sign = rand() % 2;
...
inline
i32 matrix_scale(matrix_t *m, const f64 c) {
    if (!m) {
        printf("error: invalid function argument (matrix is null)\n");
        exit(EXIT_FAILURE);
    }
